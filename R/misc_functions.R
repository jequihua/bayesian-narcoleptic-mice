...
###########################################################################################  
# Function to create data for contrasts between treatments (using "fitted").
...
tukeys_mat = function(model_input_data,brms_model_fit,columns=1:4){
  mcmc = brms_model_fit
  coefs <- as.matrix(mcmc)[, columns]
  newdata <- data.frame(x = levels(ba_seconds$group))
...
########################################################################################### 
# Plot pairwise comparitons (contrasts).
